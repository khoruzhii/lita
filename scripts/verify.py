...
SCHEME_DIR = Path(__file__).resolve().parents[1] / "schemes"
...
def dense_axis(npz, axis, rows, cols, prime):
    ...
    if mod_prime:
        values = ((numerators.astype(np.int64) % prime) * inv_mod(denominators, prime)) % prime
    else:
...
def load_factors(path, prime=None):
    with np.load(path, allow_pickle=False) as npz:
        meta = load_meta(npz)
        n1, n2, n3, rank = meta["n1"], meta["n2"], meta["n3"], meta["rank"]
        u = dense_axis(npz, "u", rank, n1 * n2, prime)
        v = dense_axis(npz, "v", rank, n2 * n3, prime)
        w = dense_axis(npz, "w", rank, n1 * n3, prime)
    return meta, u, v, w
...
def check_mod(meta, u, v, w, prime):
    n1, n2, n3 = meta["n1"], meta["n2"], meta["n3"]
    for _ in range(MOD_TRIALS):
        ...
        if not np.array_equal(got, expected):
            raise AssertionError(f"mod-prime check failed for prime={prime}")
...
for path in paths:
    meta, u, v, w = load_factors(path)
    check_float(meta, u, v, w)
    del u, v, w

    for prime in PRIMES:
        meta, u, v, w = load_factors(path, prime=prime)
        check_mod(meta, u, v, w, prime)
        del u, v, w
